...
import { Ancestry, DomainCard } from "@/types/daggerheart/daggerheart";
...
interface DomainCardDB extends DomainCard {
  uuid?: UUID; // optional so new entries get an autogenerated ID
  dataPackUUID: UUID; // reference to the data pack this card belongs to
}

interface AncestryDB extends Ancestry {
  uuid?: UUID; // optional so new entries get an autogenerated ID
  dataPackUUID: UUID; // reference to the data pack this ancestry belongs to
}

export type { DataPackDB, DomainCardDB, AncestryDB };
...
export const setup = (db: IDBPDatabase<Database>) => {
  ...
  const ancestriesStore = db.createObjectStore("ancestries", {
    autoIncrement: false,
    keyPath: ID_KEY,
  });
  ancestriesStore.createIndex("byDataPackUUID", "dataPackUUID", {
    unique: false,
  });
};
